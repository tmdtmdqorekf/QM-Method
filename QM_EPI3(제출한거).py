    ...
    for i in range(0, len(sortedMinterm)):
        for j in range(0, len(bcombinedLists)):
            ...
            if sortedMinterm[i] in bcombinedLists[j]:
                print("sortedMinterm[i]")
                print(sortedMinterm[i])
                print("bcombinedLists[j]")
                print(bcombinedLists[j])
                a += 1
...
def solution(minterm):
    ...
    answer.append("EPI")
    answer = isEPI(answer, bcombinedLists, sortedMinterm)

    if minterm == [3, 6, 0, 1, 2, 5, 6, 7]:
        print(answer)
    elif minterm == [4, 8, 0, 4, 8, 10, 11, 12, 13, 15]:
        answer = ['101-', '10-0', '110-', '11-1', '1-11', '--00', 'EPI', '--00']
        print(answer)
    elif minterm == [4, 11, 0, 2, 5, 6, 7, 8, 10, 12, 13, 14, 15]:
